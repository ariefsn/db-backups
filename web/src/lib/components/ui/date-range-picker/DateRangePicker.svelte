<script lang="ts">
	...
	let { value = $bindable(), start, end, className } = $props<{
		value?: DateRange;
        start?: string;
        end?: string;
        className?: string;
    ...
    $effect(() => {
        if (!value && start) {
            value = {
                start: start ? new CalendarDate(new Date(start).getFullYear(), new Date(start).getMonth() + 1, new Date(start).getDate()) : undefined,
                end: end ? new CalendarDate(new Date(end).getFullYear(), new Date(end).getMonth() + 1, new Date(end).getDate()) : undefined
            }
        }
    })
			...
			<RangeCalendar.Root bind:value bind:placeholder class="rounded-md border p-3">
				{#snippet children({ months, weekdays })}
					<RangeCalendar.Header class="flex w-full items-center justify-between">
						<RangeCalendar.PrevButton class="h-9 w-9 bg-transparent p-0 opacity-50 hover:opacity-100" />
						<RangeCalendar.Heading class="text-sm font-medium" />
						<RangeCalendar.NextButton class="h-9 w-9 bg-transparent p-0 opacity-50 hover:opacity-100" />
									...
									{#each month.weeks as weekDates}
										<RangeCalendar.GridRow class="flex w-full">
											{#each weekDates as date}
												<RangeCalendar.Cell {date} month={month.value} class="relative p-0 text-center text-sm focus-within:relative focus-within:z-20 [&:has([data-selected])]:bg-accent first:[&:has([data-selected])]:rounded-l-md last:[&:has([data-selected])]:rounded-r-md [&:has([data-selected][data-outside-month])]:bg-accent/50 [&:has([data-selected][data-selection-start])]:bg-accent [&:has([data-selected][data-selection-end])]:bg-accent">
													<RangeCalendar.Day
														class={cn(
